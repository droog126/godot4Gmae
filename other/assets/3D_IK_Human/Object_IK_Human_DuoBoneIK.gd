...
func _ready():
	...
	human_stuff = get_node("Human_Stuff");
	
	raycast_grounded = get_node("RayCast_Grounded");
	is_grounded = false;
	
	# START IK
	get_node("Human_Stuff/Twisted_ModifierStack3D").set("stack/execution_enabled", true);
...
func handle_movement(delta):
	...
	if (hvel.length_squared() > 0.2):
		var tmp_scale = human_stuff.scale;
		human_stuff.look_at(human_stuff.global_transform.origin - hvel, Vector3.UP);
		human_stuff.scale = tmp_scale;
...
#	if (is_grounded == true):
#		velocity = move_and_slide_with_snap(velocity, Vector3.DOWN * 1.5,  Vector3.UP)
#	else:
