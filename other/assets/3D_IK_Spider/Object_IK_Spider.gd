...
func _ready():
	...
	spider_stuff = get_node("Spider_Stuff");
	
	raycast_grounded = get_node("RayCast_Grounded");
	is_grounded = false;
	
	# START IK
	get_node("Spider_Stuff/Twisted_ModifierStack3D").set("stack/execution_enabled", true);
...
func handle_movement(delta):
	...
	if (hvel.length_squared() > 0.2):
		spider_stuff.look_at(spider_stuff.global_transform.origin - hvel, Vector3.UP);
